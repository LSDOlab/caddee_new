...
class TBWMassProperties(m3l.ExplicitOperation):
    def initialize(self, kwargs):
        # parameters
        self.parameters.declare('component', default=None, types=None)

    def compute(self):
        csdl_model = TBWMassPropertiesCSDL()
        return csdl_model
...
class TBWMassPropertiesCSDL(ModuleCSDL):
    def initialize(self):
        self.parameters.declare('name', default='tbwMP', types=str)
    ...
    def define(self):
        ...
        m = 150000.*lbs2kg + (1.2 * ar) * 0
        Ixx = 1000 + (0.34343 * ar) * 0
        Iyy = 2000 + (1.9 * ar) * 0
        Izz = 3000 + (1.7 * ar) * 0
        Ixz = 0. + (0.3 * ar) * 0

        cgx = 57.467*ft2m + (0.21 * ar) * 0
        cgy = 0. + (0.2343 * ar) * 0
        cgz = 0.593*ft2m + (0.2212 * ar) * 0
