import csdl
from caddee.caddee_core.system_model.design_scenario.design_scenario import DesignScenario
...
from caddee.csdl_core_modules.system_model_csdl.design_scenario_csdl.loads_csdl.inertial_loads_csdl import InertialLoadsModel
from caddee.csdl_core_modules.system_model_csdl.design_scenario_csdl.loads_csdl.total_forces_moments_csdl import TotalForcesMomentsCSDL
from caddee.csdl_core_modules.system_representation_csdl.mesh_evaluation_csdl import MeshEvaluationCSDL
from caddee.csdl_core_modules.system_model_csdl.mass_properties_csdl.varying_mass_properties_csdl import VaryingMassPropertiesCSDL
from caddee.csdl_core_modules.system_model_csdl.mass_properties_csdl.total_mass_properties_csdl import TotalMassPropertiesCSDL
...
class DesignScenarioCSDL(BaseModelCSDL):
    def initialize(self):
        ...
        self.parameters.declare('system_config', types=SystemRepresentation, allow_none=False)
    ...
    def define(self):
        ...
        mechanics_group, nonmechanics_group, power_group = design_scenario._assemble_groups(design_condition_dictionary)
        system_model_connections_list = design_scenario._system_model_connections_list
        psa_connections_list = design_scenario._psa_connections_list
        """
        Structure (csdl side)
        ...
        for name, design_condition in design_condition_dictionary.items():
            ...
            mission_time[counter] = self.register_module_input(f'{name}_time', shape=(1, ))
            counter += 1
